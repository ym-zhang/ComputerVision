	...
	for(int k=0; k<im.c; k++) {
		for(int y=0; y<im.h; y++) {
			for(int x=0; x<im.w; x++) {
				float val = 0;
				if (x==0&&y==0)
					val =  get_pixel(im, x, y, k);
				else if (y==0) 
					val = get_pixel(im, x, y, k) + get_pixel(integ,x-1,y,k);
				else if (x==0) 
					val = get_pixel(im, x, y, k)+get_pixel(integ, x, y-1, k);
				else
					val = get_pixel(im,x, y,k)-get_pixel(integ,x-1,y-1,k)+get_pixel(integ,x-1,y,k)+get_pixel(integ,x,y-1,k);
				set_pixel(integ,x,y,k,val);
			}
		}
	}
    return integ;
...
image box_filter_image(image im, int s)
{
	...
	for(int k=0; k<im.c; k++){
		for(int y=0; y<im.h; y++){
			for(int x=0; x<im.w; x++){
				float val=(get_pixel(integ, x+s2,y+s2,k)-get_pixel(integ, x+s2,y-s2-1,k)-get_pixel(integ,x-s2-1,y+s2,k)+get_pixel(integ,x-s2-1,y-s2-1,k))/(s*s);
				set_pixel(S,x,y,k,val);
			}
		}
	}
    return S;
...
image time_structure_matrix(image im, image prev, int s)
{
	...
	for(int y=0; y<im.h; y++) {
		for (int x=0; x<im.w; x++) {
			float Ix = get_pixel(Ix_img, x, y, 0);
			float Iy = get_pixel(Iy_img, x, y, 0);
			float It = -get_pixel(prev, x, y, 0)+get_pixel(im, x, y, 0);

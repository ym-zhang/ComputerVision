...
int same_image(image a, image b){
    ...
    for(i = 0; i < a.w*a.h*a.c; ++i){
        ...
        {
			// printf("w is %i, h is %i\n", a.w, a.h);
            // printf("The value should be %f, but it is %f! The index is %i \n", b.data[i], a.data[i], i);
			//sum+=1;
			return 0;
        }
    }
...
void test_sobel(){
    ...
    if( gt_mag.w != mag.w || gt_theta.w != theta.w || 
		...
		printf("mag true: %i thera true: %i mag :%i theta :%i\n", gt_mag.c, gt_theta.c, mag.c, theta.c);
		return;
	}
    int i;
